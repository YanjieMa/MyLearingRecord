...
void exit_init(void)
{
	...
	/* 1、设置IO复用 */
	IOMUXC_SetPinMux(IOMUXC_SNVS_SNVS_TAMPER1_GPIO5_IO01,0);			/* 复用为GPIO1_IO18 */
	...
	system_register_irqhandler(GPIO5_Combined_0_15_IRQn, (system_irq_handler_t)gpio5_io01_irqhandler, NULL);	/* 注册中断服务函数 */
 ...
 * @return 				: 无
 */
void gpio5_io01_irqhandler(void)
{ 
	...
	delay(10);
	if(gpio_pinread(GPIO5, 1) == 0)	/* 按键按下了  */
	{
		state = !state;
		beep_switch(state);
	}
